...
func (c call) Eval(env Env) float64 {
	...
	panic(fmt.Sprintf("unsupported call: %s", c.fn))
}
...
func (c call) Check(vars map[Var]bool) error {
	...
	if len(c.args) != arity {
		return fmt.Errorf("call %s has %d instead %d arguments",
...
func TestEval(t *testing.T) {
	...
	for _, test := range tests {
		...
		fmt.Printf("\t%v => %s\n", test.env, got)
		if got != test.want {
			t.Errorf("%s.Eval() in %v = %q, should be %q\n",
